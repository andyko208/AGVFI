...
model_urls = {
    ...
    "r2d_18": "https://download.pytorch.org/models/r2plus1d_18-91a641e6.pth"
}
...
class VideoResNet(nn.Module):
    ...
    def __init__(self, block, conv_makers, layers,
        ...
        self.layer2 = self._make_layer(block, conv_makers[0], 64, layers[0], stride=1)
        self.layer3 = self._make_layer(block, conv_makers[1], 128, layers[1], stride=2 , temporal_stride=1)
        self.layer4 = self._make_layer(block, conv_makers[2], 256, layers[2], stride=2 , temporal_stride=1)
        self.layer5 = self._make_layer(block, conv_makers[3], 512, layers[3], stride=1, temporal_stride=1)
...
def _video_resnet(arch, pretrained=False, progress=True, **kwargs):
    ...
    #     # Keep the relevant weights only
    #     my_dict = model.state_dict()
    #     valid_weights = ['stem', 'layer1']  # Rest layers are modified
    #     pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in valid_weights}
